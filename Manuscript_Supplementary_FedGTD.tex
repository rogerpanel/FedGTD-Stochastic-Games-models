...
\begin{abstract}
This supplementary material complements the main paper by providing comprehensive details, extended analyzes, and complete results. It includes an expanded survey of related work with additional baseline methods and recent developments; a detailed privacy analysis—covering regulatory compliance mapping and differential-privacy budget allocation strategies; and complete mathematical proofs for all theorems establishing existence, uniqueness, privacy, robustness, and convergence guarantees. We also provide extended adversarial-training procedures with domain-specific perturbation strategies; full experimental configurations to ensure reproducibility and fair baseline comparison; additional experimental results (precision–recall curves, ROC analysis, and detailed confusion matrices); scalability analyzes with computational-complexity breakdowns; and deployment considerations, including cost–benefit analysis and integration guidelines. Finally, we present a comprehensive limitations analysis with failure-mode discussions and mitigation strategies and outline several directions for future research.
\end{abstract}
...
Beyond the foundational works discussed in the main paper, several recent developments inform our approach to game-theoretic network security. Roy et al.~\cite{roy2010survey} extended classical two-player security games to multi-player scenarios, introducing security games with multiple defenders. Their analysis revealed the complexity of coordination in distributed defense, showing that Nash equilibria may be inefficient due to lack of cooperation. The work established that the price of anarchy in security games can reach order n where n is the number of defenders, highlighting the need for coordination mechanisms in federated settings. Our framework addresses this challenge through explicit coordination mechanisms built into the federated aggregation protocol in Algorithm 1 of the main paper.

The work of Alpcan and Başar~\cite{alpcan2011network} on stochastic security games provides the mathematical foundation for modeling dynamic interactions under uncertainty. They introduced saddle-point equilibria in security games, demonstrating that optimal defense strategies exist under certain regularity conditions. However, their approach assumes complete information and centralized decision-making, limitations that become critical in distributed cloud environments where each defender has only local visibility.

Recent advances by Zhu and Başar~\cite{zhu2015game} incorporate dynamic aspects through differential games, establishing Hamilton-Jacobi-Bellman equations for optimal defense strategies. Their framework models continuous-time interactions but requires complete state observability, which is unrealistic in multi-cloud environments. Moreover, their computational approach scales exponentially with state space dimension, limiting practical applications. Our jump-diffusion formulation in Equations 6 through 8 of the main paper provides a more scalable alternative.

\subsection{A.2 Advanced Federated Learning for Security}

Preuveneers et al.~\cite{preuveneers2018federated} first applied federated learning to intrusion detection, achieving 97.8\% accuracy on the NSL-KDD dataset. Their approach demonstrated the feasibility of training distributed security models, but did not consider adversarial participants, privacy guarantees, or the heterogeneous characteristics of modern cloud environments.

Wang et al.~\cite{wang2024cloudfl} introduced CloudFL, a federated learning framework specifically designed for multi-cloud security, achieving 92.1\% accuracy. However, CloudFL does not incorporate game-theoretic considerations to model strategic adversarial behavior and assumes relatively mild heterogeneity compared to the extreme diversity in ICS3D datasets.

Chen et al.~\cite{chen2024adaptive} developed AdaptiveFL-Container with 89.7\% detection rates for container security, introducing domain adaptation techniques. Their approach focuses specifically on container security and does not generalize to other cloud domains. Liu et al.~\cite{liu2024hierarchical} proposed hierarchical federated learning for IoT security, demonstrating 91.3\% accuracy. Their hierarchical architecture addresses scalability concerns, but does not provide Byzantine resilience guarantees.

\subsection{A.3 Extended Byzantine Robustness Analysis}

The vulnerability of federated learning to Byzantine attacks was first systematically studied by Blanchard et al.~\cite{blanchard2017machine}, who showed that a single malicious client can arbitrarily manipulate the global model in standard FedAvg implementations. Their Krum algorithm provides some robustness, but fails under coordinated attacks.

Yin et al.~\cite{yin2018byzantine} introduced coordinate-wise median and trimmed mean aggregation, achieving breakdown points near 50\%. However, their analysis assumes homogeneous data distributions and may fail when natural heterogeneity across domains mimics Byzantine behavior.

Recent advances include the BALANCE algorithm by Zhang et al.~\cite{zhang2024balance}, which enables each client to use its local model as a similarity reference. BALANCE achieves strong theoretical guarantees under IID assumptions, but degrades with extreme class imbalance common in security applications.

Zhou et al.~\cite{zhou2024robust} introduced RobustFL with 94.2\% accuracy under 30\% Byzantine corruption. Patel et al.~\cite{patel2024secure} developed SecureAgg+ achieving 93.8\% robustness with improved communication efficiency. Singh et al.~\cite{singh2024byzantine} proposed Byzantine-resilient federated learning for edge computing with 92.6\% performance retention.
...
The proof follows from advanced composition theorems for differential privacy. Each federated round adds independent Gaussian noise calibrated to the privacy budget \(\epsilon_d\). In adaptive composition, the total loss of privacy increases as \(O(\sqrt{T})\) rather than linearly, due to the strong composition properties of the Gaussian mechanism.
...
\]
The imbalance-adjusted clipping norm accounts for the varying magnitudes of the gradient between domains with different imbalance ratios.
...
Under the Lipschitz conditions, the expected supremum-norm squared of successive differences satisfies $\mathbb{E}[\sup_{u \leq t}\|\Delta^{(n)}_u\|^2] \leq C \int_0^t \mathbb{E}[\|\Delta^{(n-1)}_u\|^2]\,du$. By Gr\"onwall's inequality, this establishes convergence of the Picard sequence.
...
For the convergence rate, summing the descent inequality over rounds with learning rate of order $O(1/t^{2/3})$ yields sums that evaluate to $O(T^{1/3})$, establishing the stated rate.
...
The domain-adaptive learning rate formula \(\eta_d^{(t)}=\eta_0\,\rho_d^{1/2}/(t+1)^{2/3}\) emerged from both theoretical analysis and empirical validation. From convergence analysis in Theorem 4, the optimal learning rate decay for non-convex objectives with heterogeneous data follows $O(1/t^{\alpha})$ where $\alpha \in [1/2,\, 2/3]$. The two-thirds power provides best balance between exploration and exploitation.
...
The Dirichlet concentration parameter $\alpha = 0.3$ for data partitioning simulates realistic heterogeneity observed in multi-organizational deployments. Preliminary studies comparing values of $\alpha$ revealed that $\alpha = 0.3$ best matches the label-distribution divergence observed in real-world federated security systems. Jensen--Shannon divergence shows $0.47 \pm 0.08$ for $\alpha = 0.3$, compared to $0.42 \pm 0.05$ observed in production deployments.

Domain-specific privacy budgets were chosen through consultation with industry partners and regulatory compliance experts. SOC environments with $\epsilon_{\text{SOC}} = 1.8$ represent the most sensitive domain owing to entity relationships, chosen to satisfy European Data Protection Board guidance suggesting $\epsilon < 2$ for personal data. Container environments with $\epsilon_{\text{Container}} = 2.0$ reflect moderate sensitivity. Edge-IIoT environments with $\epsilon_{\text{Edge}} = 2.5$ face less stringent requirements given the predominantly operational nature of the data.
...
Statistical testing methodology ensures robustness. Each reported result represents the mean $\pm$ standard deviation across five independent runs initialised with seeds $\{42, 123, 234, 345, 456\}$. Paired $t$-tests compare methods on identical data splits. A Bonferroni correction adjusts the significance threshold to $\alpha' = 0.05 / m$, where $m$ denotes the number of pairwise comparisons, to control the family-wise error rate. Cohen's $d$ quantifies practical effect size. All reported improvements satisfy $p < 0.01$ after Bonferroni correction with $d > 2.5$.
...
The Edge-IIoT domain with moderate $2.67{:}1$ imbalance shows FedGTD achieving PR-AUC of $0.937$ compared to $0.891$ for RobustFL and $0.847$ for FedAvg, representing a $5.2\%$ improvement over the best baseline. The Container domain with severe $15.7{:}1$ imbalance shows FedGTD achieving PR-AUC of $0.924$ compared to $0.878$ for RobustFL, representing a $5.2\%$ improvement. The SOC domain with extreme $99{:}1$ imbalance shows the most pronounced improvement, with FedGTD achieving PR-AUC of $0.687$ compared to $0.531$ for RobustFL, a $29.4\%$ relative gain that validates the effectiveness of imbalance-weighted objectives under extreme conditions.

\subsection{F.2 Detailed Confusion Matrices}

The Edge-IIoT confusion matrix reveals balanced performance across all 14 attack families despite severe class imbalance. Rare attacks such as XSS ($0.8\%$ of data) achieve $88.7\%$ recall, while common attacks such as DDoS maintain $96.4\%$ recall. The primary confusion occurs between similar attack types; for example, SQL Injection and XSS show $1.2\%$ cross-confusion, reflecting semantic similarity in web-based exploit patterns. Normal traffic achieves $97.2\%$ precision, demonstrating a low false-positive rate suitable for operational deployment.

The Container confusion matrix demonstrates precise CVE-specific detection with minimal cross-confusion. Container escape CVE-2019-5736 achieves $96.7\%$ recall with only $1.1\%$ confusion with path traversal attacks, where both involve privilege escalation mechanisms explaining limited overlap. The $2.2\%$ false-positive rate on benign traffic represents acceptable operational overhead for critical container security.

The SOC confusion matrix addresses the most challenging scenario with 99:1 overall imbalance where true positive incidents represent only $0.8\%$ of all incidents. FedGTD achieves $92.3\%$ recall on true positives, substantially exceeding RobustFL at $68.8\%$. Critically, $95.6\%$ of misclassified true positives are labeled as benign positive rather than false positive, ensuring these incidents still receive analyst attention. This confusion pattern aligns with operational SOC workflows where distinguishing true positive from benign positive requires detailed investigation.

\subsection{F.3 Statistical Significance Testing}

All tests compare FedGTD against RobustFL (best baseline) on identical data splits with five independent runs. For Edge-IIoT accuracy: $\text{df}=4$, $t=8.34$, $p=0.0012$, $d=3.73$. For Container accuracy: $t=7.21$, $p=0.0019$, $d=3.22$. For SOC accuracy: $t=12.47$, $p=0.0002$, $d=5.57$.

For Edge macro-F1: $t=9.12$, $p=0.0008$, $d=4.08$. For Container macro-F1: $t=6.89$, $p=0.0024$, $d=3.08$. For SOC macro-F1: $t=11.23$, $p=0.0003$, $d=5.02$.

For Byzantine retention at $20\%$: $t=5.67$, $p=0.0047$, $d=2.53$. For communication efficiency: $t=14.32$, $p=0.0001$, $d=6.40$. All improvements remain statistically significant after Bonferroni correction for $m=12$ comparisons (adjusted threshold $\alpha'=0.00417$). Cohen's $d$ values ranging from $2.53$ to $6.40$ indicate very large practical significance.
...
The per-client training cost for $E$ local epochs over a dataset of size $n_k^{(d)}$ with model dimension $d$ and $L$ layers is $O(E\,n_k^{(d)}\,d\,L)$. Gradient clipping requires $O(d)$ operations. Byzantine detection via pairwise gradient similarities requires $O(K^{2}d)$ operations, constituting the primary scalability bottleneck. Trimmed-mean aggregation requires $O(d\,K\log K)$ operations.
...
We conducted experiments assessing scalability to larger federations with $K \in \{20, 40, 60, 80, 100\}$ participants. Detection accuracy improves modestly from 96.1\% at $K=20$ to 96.6\% at $K=100$, a gain of 0.5 percentage points attributable to broader attack coverage. Per-round wall-clock time grows approximately quadratically from 39.7\,s at $K=20$ to 358.1\,s at $K=100$; fitting a power law yields an exponent of 1.95, closely matching the theoretical $O(K^2)$ complexity of pairwise Byzantine detection.

Communication overhead per round grows linearly from 1.92\,GB at $K=20$ to 9.60\,GB at $K=100$, consistent with the $O(K)$ scaling of gradient exchange. Byzantine detection time alone grows quadratically from 15.3\,s to 467.3\,s over the same range, confirming this component as the primary scalability bottleneck. Memory usage grows approximately linearly from 15.2\,GB to 72.3\,GB.

At $K=100$ participants the per-round time of ${\sim}\,358$\,s (${\approx}\,6$\,min) remains acceptable for deployment scenarios in which federated rounds occur hourly or daily. For more frequent update cycles or federations exceeding $K=100$, the approximation strategies described in Section~G.3 become necessary.

\subsection{G.3 Proposed Scalability Improvements}

A hierarchical federation architecture reduces the $O(K^2 d)$ Byzantine detection cost through multi-tier organisation. At Level~1, clients are partitioned into clusters of size $k \approx \sqrt{K}$, and each cluster performs Byzantine detection locally at cost $O(k^2 d)$. At Level~2, regional coordinators receive cluster aggregates and carry out a second-level detection step at cost $O(K\,d)$. The overall complexity therefore drops from $O(K^2 d)$ to $O(K\,d)$, enabling scaling to thousands of participants.

Approximate Byzantine detection offers a complementary path. Locality-sensitive hashing (LSH) projects each gradient vector from $\mathbb{R}^d$ into a lower-dimensional space $\mathbb{R}^{d'}$ with $d' \ll d$, after which pairwise similarities are computed at cost $O(K\,d')$ instead of $O(K^2 d)$. Alternatively, random sampling has each client compare its gradient against $s \ll K$ randomly selected peers, reducing the cost to $O(K\,s\,d)$; choosing $s = O(\log K)$ retains high detection probability while keeping overhead logarithmic in the federation size.

Preliminary experiments with LSH-based approximation using $d' = O(\log d)$ show a 2--3 percentage-point degradation in Byzantine detection accuracy but enable scaling to $K = 500$ with per-round times under 120\,s, representing a $50{\times}$ speedup over exact pairwise similarity computation.
...
Quantified annual benefits include false positive reduction saving 120000 to 250000 dollars through reduced analyst workload, with a $67\%$ reduction in false positives directly translating to proportional reduction in investigation time. Faster incident response saves 60000 to 140000 dollars through a $23\%$ reduction in mean time to respond. Novel attack detection saves 180000 to 450000 dollars through an $89\%$ improvement in detecting previously unseen attack variants. Compliance automation saves 30000 to 75000 dollars through reduced manual audit effort. Reduced breach risk quantified at 100000 to 250000 dollars annually accounts for decreased probability of costly security incidents. Total annual benefits range from 490000 dollars at Site B to 1165000 dollars at Site C.

Return on investment, calculated as $(\text{annual benefit} - \text{initial cost} - \text{annual cost})\,/\,(\text{initial cost} + \text{annual cost})$, yields $1{,}018\%$ at Site~B, $1{,}380\%$ at Site~A, and $1{,}248\%$ at Site~C. All sites achieve positive ROI within first year with break-even occurring at 4 to 5 months.
...
Our $72$--$76\%$ detection rate for zero-day attacks reflects fundamental limitations of supervised learning approaches. For signature-based approaches, theoretical detection probability for zero-day attacks is bounded by expression depending on model capacity and feature overlap. Under reasonable assumptions of $30$--$40\%$ feature overlap, this yields a detection probability in the range $60$--$65\%$. FedGTD achieves $72$--$76\%$, exceeding theoretical bound by leveraging cross-domain knowledge transfer.
...
Lattice-based cryptography offers viable solutions with proposals to replace RSA authentication using NIST-approved CRYSTALS-Dilithium signatures and employ Kyber for key encapsulation in secure aggregation. These replacements introduce expected performance overhead of $20$--$30\%$ increase in computation time. Hash-based signatures, particularly SPHINCS+ for one-time client commitments and hash chains for reputation score verification impose minimal overhead.
